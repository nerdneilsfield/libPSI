...
int main(int argc, char **argv)
{
  ...
  if ((result == TestCollection::Result::skipped && hasProtocolTag == false && cmd.isSet(DcwrTags) == false && cmd.isSet(rr16Tags) == false && cmd.isSet(rr17aTags) == false && cmd.isSet(rr17aSMTags) == false && cmd.isSet(rr17bTags) == false && cmd.isSet(kkrtTag) == false && cmd.isSet(drrnTag) == false && cmd.isSet(grrTags) == false && cmd.isSet(dktTags) == false && cmd.isSet(ecdhTags) == false && cmd.isSet(pingTag) == false) || cmd.isSet(helpTags)) {
    std::cout << Color::Red
              << "#######################################################\n"
              << "#                      - libPSI -                     #\n"
